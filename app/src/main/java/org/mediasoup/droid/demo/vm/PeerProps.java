...
public class PeerProps extends PeerViewProps {
    ...
    public PeerProps(@NonNull Application application, @NonNull RoomStore roomStore) {
        ...
        mStateComposer.addOnPropertyChangedCallback(new Observable.OnPropertyChangedCallback() {
            ...
            public void onPropertyChanged(Observable sender, int propertyId) {
                ...
                mAudioRtpParameters.set(audioConsumer != null ? audioConsumer.getRtpParameters() : null);
                mVideoRtpParameters.set(videoConsumer != null ? videoConsumer.getRtpParameters() : null);
                ...
                mAudioEnabled.set(audioCW != null && !audioCW.isLocallyPaused() && !audioCW.isRemotelyPaused());
                mVideoVisible.set(videoCW != null && !videoCW.isLocallyPaused() && !videoCW.isRemotelyPaused());
            }
        });
    }
    ...
    public void connect(LifecycleOwner owner, @NonNull String peerId) {
        ...
        getRoomStore().getRoomInfo().observe(owner, roomInfo -> mFaceDetection.set(roomInfo.isFaceDetection()));
    ...
    public static class StateComposer extends BaseObservable {
        ...
        private Observer<Peers> mPeersObservable = peers -> {
            mPeer = peers.getPeer(mPeerId);
            Logger.w(TAG, "onChanged() id: " + mPeerId + ", name:" + (mPeer != null ? mPeer.getDisplayName() : ""));
            // TODO(HaiyangWu): check whether need notify change.
            notifyChange();
        };

        private Observer<Consumers> mConsumersObserver = consumers -> {
