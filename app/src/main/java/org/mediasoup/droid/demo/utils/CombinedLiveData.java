...
public class CombinedLiveData<A, B, OUTPUT> extends MediatorLiveData<OUTPUT> {
    ...
    public CombinedLiveData(LiveData<A> source1, LiveData<B> source2, @NonNull Combiner<A, B, OUTPUT> combiner) {
        super.addSource(source1, (data1) -> {
            input1 = data1;
            setValue(combiner.combine(input1, mB));
        });

        super.addSource(source2, (data2) -> {
            mB = data2;
            setValue(combiner.combine(input1, mB));
        });
    }
